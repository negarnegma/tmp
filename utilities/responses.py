...
class SuccessResponse(BaseResponse):
    def __init__(self, data='', message='', status=200, **kwargs):
        self.data = data
        self.message = message
        self.current_time = round(time.time())
        self.success = True
...
class ExceptionHandlerMiddleware(object):
    ...
    def process_template_response(request, response):
        if response.status_code == 200 and hasattr(response, 'data') and isinstance(response.data, dict):
            ...
            response.data = {'success': False, 'message': str(_('Permission denied')), 'code': response.status_code,
            ...
            # 406 Not Acceptable
            data = {
                'errors': response.data, 'message': str(_('Not Acceptable request, maybe the version is not supported')),
                'dev_error': {'user': request.user.username, 'api': request.path},
            ...
            data = {
                'errors': response.data, 'message': str(_('Unsupported media type')),
                'dev_error': {'user': request.user.username, 'api': request.path},
                'show_type': settings.MESSAGE_SHOW_TYPE['NONE']
            }
